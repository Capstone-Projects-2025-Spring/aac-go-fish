...
jest.mock('../components/Drinks/DrinkDisplay', () => ({ color, fillPercentage, cupSize, cupPosition }) => (
  <div data-testid="drink-display" data-color={color} data-fill={fillPercentage} data-size={cupSize} data-position={cupPosition} />
));
...
describe('DrinkBuilder', () => {
  ...
  it('selects color and plays audio', () => {
    renderBuilder();
    const mediumBtn = screen.getByAltText('Medium Cup').closest('button');
    ...
    const display = screen.getByTestId('drink-display');
    expect(display).toHaveAttribute('data-color', '#FF0000');
  });

  it('selects cup size and plays audio', () => {
    renderBuilder();
    const smallBtn = screen.getByAltText('Small Cup').closest('button');
  ...
  it('fills cup while holding fill button', () => {
    ...
    const display = screen.getByTestId('drink-display');
    expect(display).toHaveAttribute('data-fill', '5');
    fireEvent.mouseUp(fillBtn);
    const fillingAudio = global.Audio.mock.results.find(r => r.value.src === '/audio/filling.mp3').value;
  ...
  it('sends drink and shows confirmation', () => {
    ...
    expect(screen.queryByText('Drink sent to manager!')).toBeNull();
  });
});
