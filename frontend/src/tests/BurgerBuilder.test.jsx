...
jest.mock('../menuItems', () => ({ menu: [{ children: [
  { name: 'Ingredient1', image: 'img1.png', sideImage: 'side1.png', audio: '/audio/ingredient1.mp3' },
  { name: 'Ingredient2', image: 'img2.png', sideImage: 'side2.png', audio: '/audio/ingredient2.mp3' }
] }] }));
...
describe('BurgerBuilder', () => {
  ...
  it('shows and hides StationStartModal after 10s', () => {
    renderBuilder();
  ...
  it('adds ingredients, plays sounds, and updates station', () => {
    renderBuilder();
    const btn1 = screen.getByText('Ingredient1');
    fireEvent.click(btn1);
    expect(playPopSound).toHaveBeenCalled();
    expect(global.Audio).toHaveBeenCalledWith('/audio/ingredient1.mp3');
    const station = screen.getByTestId('burger-station');
    expect(station).toHaveAttribute('data-paths', JSON.stringify(['side1.png']));
  });
  ...
  it('clears plate on delete', () => {
    renderBuilder();
    fireEvent.click(screen.getByText('Ingredient1'));
    const clear = screen.getByText('Delete Burger');
    fireEvent.click(clear);
    const station = screen.getByTestId('burger-station');
    expect(station).toHaveAttribute('data-paths', JSON.stringify([]));
  });

  it('sends order and plays send sound', () => {
    renderBuilder();
    fireEvent.click(screen.getByText('Ingredient1'));
    const sendBtn = screen.getByAltText('Send Order').closest('button');
    fireEvent.click(sendBtn);
    expect(sendMock).toHaveBeenCalledWith({ data: { type: 'game_state', game_state_update_type: 'order_component', component_type: 'burger', component: { ingredients: ['Ingredient1'] } } });
